...
let
  ...
  wrapper =
    ...
    let
      ...
      makeLuaPath = drv: [
        "${luaPathPrefix drv}/?/init.lua"
        "${luaPathPrefix drv}/?.lua"
      ];
      makeLuaCPath = drv: [
        "${luaCPathPrefix drv}/?/init.so"
        "${luaCPathPrefix drv}/?.so"
      ];
      luaEnv = luajit.withPackages (ps: concatMap (f: f ps) cfg.luaModules);

      fullNvimWrapperArgs = baseNvimWrapperArgs ++ [
        "--add-flags"
        ''"--cmd \"luafile ${cfg.neovimRC}\""''
        "--argv0 nvim"
      ...
      makeSuffixArg =
        var: sep: val:
        "--suffix ${escapeShellArg var} ${escapeShellArg sep} ${escapeShellArg val}";
      makeSuffixListArg =
        var: sep: list:
        makeSuffixArg var sep "${concatStringsSep sep list}";
